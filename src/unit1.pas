...
begin
  if x = 1 then
   begin
    if ((form1.Image1.Width-step_scroll) > 400) and ((form1.Image1.Height-step_scroll) > 400) then
     begin
      form1.Image1.Width:=form1.Image1.Width-step_scroll;
      form1.Image1.Height:=form1.Image1.Height-step_scroll;
     end;
   end;
  if x = 2 then
   begin
    if ((form1.Image1.Width+step_scroll) < original_width) then
     if ((form1.Image1.Width+step_scroll) <= (Screen.Width-32)) and (form1.Height <> Screen.Height-32) then
       form1.Image1.Width:=form1.Image1.Width+step_scroll;
    if ((form1.Image1.Height+step_scroll) < original_height) then
     if ((form1.Image1.Height+step_scroll) <= (Screen.Height-32)) and (form1.Height <> Screen.Width-32) then
       form1.Image1.Height:=form1.Image1.Height+step_scroll;
   end;
...
begin
  ...
  try
    ...
    original_width := form1.Image1.Width;
    original_height := form1.Image1.Height;
    ...
    if (form1.Image1.Picture.Height >= Screen.Height) or (form1.Image1.Picture.Width >= Screen.Width) then
     begin
      ...
      if form1.Image1.Picture.Width >= Screen.Width then
       form1.Image1.Width := Screen.Width-32;
      if form1.Image1.Picture.Height >= Screen.Height then
       form1.Image1.Height := Screen.Height-32;
     end;
    set_position;
...
begin
  ...
  if key = VK_F1 then //help
   form2.ShowModal;
  if (key = VK_I) and (form1.Image1.Picture.Graphic <> nil) then //Information
   form3.ShowModal;
  if key = VK_RIGHT then //right image
    move_picture('right');
  if key = VK_LEFT then //left image
    move_picture('left');
  if key = VK_OEM_MINUS then //left image
    scroll(1);
  if key = VK_OEM_PLUS then //left image
    scroll(2);
end;
